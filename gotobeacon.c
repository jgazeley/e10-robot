...
#pragma config(Sensor, dgtl6, armBumper,			sensorTouch)  // bumper switch to detect arm contact
...
#define FIND_RED								1
#define TURN_OFF_RED						2
#define FIND_GREEN							3
#define CAPTURE_GREEN						4
#define EXIT_ARENA 							5
#define DONE										6

// GROUP 4 beacon frequencies and motor speeds
...
void lowerArm() {
  ...
  while(!SensorValue[armBumper]) {}           // motor spins until arm contacts the top of the beacon
  motor[armMotor] = ZERO_SPEED;
}

// raise the arm after pressing red beacon button or to release green beacon
void raiseArm() {
  motor[armMotor] = HALF_SPEED_REVERSE; // reverse to raise arm
  delay(2000);   // <------ REPLACE WITH LIMIT SWITCH?
  motor[armMotor] = ZERO_SPEED;
}
